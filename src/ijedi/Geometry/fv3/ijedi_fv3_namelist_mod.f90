...
implicit none
private
public ijedi_fmsnamelist

type ijedi_fmsnamelist
...
! Arguments
class(ijedi_fmsnamelist), intent(inout) :: self
...
if(.not. file_exists)then
  call abor1_ftn("ijedi_fv3_namelist_mod.file_to_namelist "//namelist_filename//" does not exist")
endif
...
ierr = check_nml_error(ios,'ijedi geom writing input_nml_file')
...
class(ijedi_fmsnamelist), target, intent(inout) :: self
...
type(ijedi_fmsnamelist), intent(inout) :: self
call self%revert_namelist()
end subroutine dummy_final

! --------------------------------------------------------------------------------------------------

end module ijedi_fv3_namelist_mod
